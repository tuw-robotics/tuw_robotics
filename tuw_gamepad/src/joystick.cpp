...
int Joystick::init(std::string &msg)
{
    ...
    if (joystick_fd == -1)
    {
        ...
        ss << "Make sure the joystick is connected and you have permissions to read " << device_ << std::endl;
...
int Joystick::wait_of_event(std::string &msg)
{
    ...
    else
    {
        if (bytes_read == sizeof(js_event))
        {
            ...
            // Process the event
            switch (js_e.type & ~JS_EVENT_INIT)
            { // Mask out JS_EVENT_INIT to handle initial states
