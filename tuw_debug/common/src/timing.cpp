...
std::string Timing::info(){
    ...
    for (size_t i = 1; i < measurments_.size(); i++){
        ss << measurments_[i].first << " " << (measurments_[i].second - measurments_[i-1].second).count() << std::endl;
    }
    return ss.str();
}
...
std::string Timing::plot(){
    ...
    for (size_t i = 1; i < measurments_.size(); i++){
        ...
        ss << std::setw(20) << measurments_[i].first << " " << std::setw(20) << std::fixed << std::setprecision(6) << d_m  << " ms: ";
